...
const io = socketio(server, {
    cors: {
        origin: 'https://chat-react-aplication.netlify.app',
        methods: ['GET', 'POST'],
        credentials: true,
    },
});
...
io.on('connection', (socket) => {
    socket.on('join', ({ name, room }, callback) => {
        ...
        // emite a todos menos al mismo socket cliente
    ...
    socket.on('sendMessage', (message, callback) => {
        ...
        // emite a todos incluso al mismo socket cliente
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            // after - io.to(user.room).emit('message', { user: 'admin', text: `${user.name} has left` });
            io.to(user.room).emit('roomData', { room: user.room, users: getUsersInRoom(user.room) });
